//
//  PremierLevelCollectionViewController.swift
//  BrainFuck
//
...
class PremierLevelCollectionViewController: UICollectionViewController {
    ...
    {
        return (self.sauvegarde.integerForKey("PremierNumberOfLevelCompleted") + 1)
    }
    
    internal func levelCompleted(level: Int)
    {
        if (level > self.sauvegarde.integerForKey("PremierNumberOfLevelCompleted"))
        {
            self.sauvegarde.setInteger(level, forKey:"PremierNumberOfLevelCompleted");
    ...
    {
        let premierGameViewController = PremierGameViewController()
        
        premierGameViewController.level = level
        premierGameViewController.premierLevelCollectionViewController = self
        premierGameViewController.endlessMod = self.endlessMod
        
        self.navigationController?.pushViewController(premierGameViewController, animated:true)
    }
    ...
    override func collectionView(collectionView: UICollectionView, didSelectItemAtIndexPath indexPath: NSIndexPath) {
        if (self.endlessMod)
        {
            ...
            let alertActionScore = UIAlertAction(title:"Scores", style:.Default) { (_) in
                ...
                scoreTableViewController.identifier = "PremierLevelN°" + String(indexPath.row + 1)
                
                scoreTableViewController.theTitle = "Premier : Scores"
