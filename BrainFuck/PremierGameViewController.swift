//
//  PremierGameViewController.swift
//  BrainFuck
//
...
class PremierGameViewController: UIViewController {

    internal var level = 0
    
    internal var premierLevelCollectionViewController = PremierLevelCollectionViewController()
    ...
    override func viewDidLoad() {
        ...
        self.title = "Trouver le nombre premier"
        ...
        self.questionLabel.text = "Trouver le nombre premier parmi les nombres suivants"
    ...
    private func setQuestion()
    {
        ...
        while (i < limit)
        {
            ...
            while (self.thereIsPremier() && self.isPremier(a))
            {
        ...
        if (!self.thereIsPremier())
        {
            let indice = arc4random_uniform(UInt32(limit))
            var a = self.getRandomNumber()
            while (!self.isPremier(a))
            {
                a = self.getRandomNumber()
            }
    ...
    private func correctAnswer()
    {
        ...
        if (self.objectif == 0)
        {
            self.premierLevelCollectionViewController.levelCompleted(self.level)
            if (!self.endlessMod)
            {
    ...
    private func thereIsPremier() -> Bool
    {
        var i = 0
        while (i < self.choixMultiplesCollectionViewController.getNumberOfItems())
        {
            if (self.isPremier(Int(self.choixMultiplesCollectionViewController.getItem(i) as String)!))
            {
                return true
            }
            i += 1
    ...
    private func isPremier(number: Int) -> Bool
    {
        var i = 2
        let racineCarree = sqrt(Double(number))
